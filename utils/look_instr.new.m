  ...
  while LEV
    ...
    if REPLOT, REPLOT--; LEV++;		% skip / special cases loop over
      ...
      k = kbhit(); printf(k);
		...
		OLDLEV=	LEV;
      if k=='q',	LEV=0;			% close all delayed to end
...
%	{color | colour | monochrome}	{blacktext | colortext | colourtext}
%	{solid | dashed} {dashlength | dl <DL>}
%	{linewidth | lw <LW>}		{<duplexing>}
%	{rounded | butt}		{fontfile [add | delete] "<filename>"}
%	{palfuncparam <samples>{,<maxdeviation>}}
%					{"<fontname>"} {<fontsize>}

%   gset('terminal','postscript','eps','color')		% pdf ?
%   gset( 'output' ,'"look_instr.eps"')
%   replot
