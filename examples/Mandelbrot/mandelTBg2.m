function mandelTBg2(coords,pixels,max_it)
%MANDELTBG2	Mandelbrot set program, parallel version (taskbag, by lines)
%
%  mandelTBg2 [ (coords [ ,pixels [ ,max_it ]] ) ]	can't be run sequential
%
%  Designed to oversubscribe master node, who owns TBag
%	...and who also works, but in 2 different processes
%
%  Example:	for running the minimum process config from UNIX cmdline, use
%
%  mpirun n0,0-0 -x DISPLAY octave -q --eval \
%	      "global FIGURES, FIGURES=true; mandelTBg2([-2-2i, 2+2i])"
%
%  mpirun n0,0-4	    octave -q --eval mandelTBg2
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%			Designed to oversubscribe master node, which owns TBag
%			...and also works, but in 2 different processes
%
%			for running minimum process from UNIX cmdline, use
%  mpirun n0,0-0     matlab -nodisplay -nosplash -nojvm -r mandelTBg2
%  mpirun n0,0-0     octave                      -q --eval mandelTBg2
%
%			for running several processes from UNIX cmdline, use
%  mpirun n0,0-<n-1> matlab -nodisplay -nosplash -nojvm -r mandelTBg2
%  mpirun n0,0-<n-1> octave                      -q --eval mandelTBg2
%
%			for running a scalability test 1..8PC's from bash, use
%  for i in 0 1 2 3 4 5 6 7; do mpirun n0,0-$i ... ; done
%                                                               or better
%  for t in 1 2 3; do for i in 0 1 2 3 4 5 6 7; do \		or also
%  for i in 0 1 2 3 4 5 6 7; do for t in 1 2 3; do
%	echo ___ $i Processes ___ time $t ____
%	mpirun n0,0-$i ...
%	sleep 1					to avoid tbox cache errors
%       done;done >     mandelTBg2.<Oct|MLAB>.1-8x3.log &	or also
%       done;done | tee mandelTBg2.<Oct|MLAB>.1-8x3.log
%-------------------------------
...
%	lcl=0;				% local #tasks for MAST % NO! f/rank1 %
...
%   TaskCount(slv)=  1;			% this can be done at once later
  ...
  % TaskBag loop % variant: if no slaves pending, wait
  %%%%%%%%%%%%%%%% master node is oversubscribed (ranks0-1), rank 1 is working
  while flght>0%|| col<x_siz	% no special case for 1 master/no slaves
...
% fprintf('n%02d%5d (local)\n',MAST,lcl);	if nprc>1
  fprintf('n%02d%5d (local)\n',   1,TaskCount(1));	if nprc>2
  fprintf('n%02d%5d\n',   [2:nprc-1;TaskCount(2:end)]);	end
