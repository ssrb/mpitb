...
//      int i, j, k;
	...
	XSync(display, 0);
        ...
        for(j=0; j < Y_RESN; j++) {
          ...
          z.real = z.imag = 0.0;	/* scale factors for 800 x 800 window */
	  ...
	  c.imag = ((float) i - 400.0)/200.0;
          ...
          } while (lengthsq < 4.0 && k < 32000); // 100);

	XSetForeground(display,gc,       k);
//	XSetForeground(display,gc,(long) k);
//	long l= Y_RESN; l=l*i+j;printf("%ld·",l);
