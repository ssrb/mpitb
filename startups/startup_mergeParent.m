...
% STARTUP_MERGEPARENT	Script for children MPITB tasks (used in TUTORIAL)
%
%	It will be called from the last else branch in startup_MPITB
%		if this is a spawned task (detected by LAM/OMPI envvar)
%	Programmed as script (not function) to have NEWORLD ready to use
%		some variables are defined: WORLD, NEWORLD, RANK, HOSTNAME
...
if isempty(getenv('LAMPARENT')) &&...
   isempty(getenv('OMPI_MCA_universe'))
	error('startup_mergeParent: only for spawned tasks')
end
...
info=MPI_Init;
if info~=MPI_SUCCESS
	error('startup_mergeParent: must lamboot first')
end
