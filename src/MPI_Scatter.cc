...
\n\
  SEE ALSO: MPI_Barrier, MPI_Bcast, MPI_Gather, MPI_Reduce\n\
            collective\n\
\n\
  MPI_Scatter is a collective operation on comm (all ranks must call it)\n\
\n\
  it is enough that meaningless arguments be the same type at non-root ranks\n\
  for instance, MPI_Scatter([],R,0,NEWORLD) if scattering doubles\n\
\n\
")
