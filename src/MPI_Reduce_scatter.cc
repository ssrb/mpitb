...
## ----------------------------------------------------
 */
#define   NAME  MPI_Reduce_scatter
/*
 * ----------------------------------------------------
...
\n\
  ...
  cnts(intvec) number of elements in result distributed to each process\n\
               cnts must be identical on all calling processes\n\
\n\
...
\n\
  SEE ALSO: MPI_Barrier, MPI_Bcast, MPI_Scatter, MPI_Gather, MPI_Reduce\n\
            collective\n\
\n\
  MPI_Reduce_scatter is collective operation on comm (all ranks must call it)\n\
\n\
...
")
	...
	PATN_SRCOC (NAME)	// SRCOC -> src/rcvar, cnt, op, comm
